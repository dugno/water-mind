...
class SimpleWaterCupPainter extends CustomPainter {
  ...
  void _drawRuler(Canvas canvas, Size size) {
    ...
    // Vẽ thước đo bên phải cốc
    final rulerX = cupLeft + cupWidth + 10;
    
    // Vẽ đường thẳng chính của thước đo
    canvas.drawLine(
      Offset(rulerX, cupTop),
      Offset(rulerX, cupTop + cupHeight),
      rulerPaint,
    );
    ...
    // Vẽ các vạch chia lớn (mỗi 200ml)
    for (int i = 0; i <= 5; i++) {
      final y = cupTop + cupHeight - (i / 5) * cupHeight;
      final value = (i * 200).toInt();
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(rulerX + 10, y - textPainter.height / 2));
    }
    
    // Vẽ các vạch chia nhỏ (mỗi 100ml)
    for (int i = 1; i < 10; i++) {
      if (i % 2 == 0) continue; // Bỏ qua các vạch chia lớn
      
      final y = cupTop + cupHeight - (i / 10) * cupHeight;
